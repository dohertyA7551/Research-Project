...
export_proteomics_limma_volcano <- function(
  ...
  df <- results %>%
    ...
    dplyr::mutate(
      gene = .data$feature,
      label = clean_feature_label(.data$feature),
  ...
  has_fdr <- any(df$adj.P.Val <= fdr_cutoff, na.rm = TRUE)
  label_df <- df %>%
  ...
  ymax <- max(2, max(df$neg_log10_adjP, na.rm = TRUE) * 1.08)

  p <- ggplot2::ggplot(
    df,
    ggplot2::aes(x = logFC, y = neg_log10_adjP, colour = fc_class)
  ) +

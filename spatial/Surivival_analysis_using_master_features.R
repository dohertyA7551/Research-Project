...
meta_markers <- read_csv("/work_space/files/Features/Run2_all/Manual_classes/act4_metacluster_markers.csv")
names(meta_markers)
...
n_patients <- ss_patient_map %>%
  distinct(Patient) %>%   # or distinct(Patient) if you kept that name
  nrow()

n_patients

##squish clin data to one row per patient
clin_patient <- clin_all %>%
  group_by(Patient) %>%
  summarise(
    across(
      .cols = everything(),
      .fns  = ~ dplyr::first(na.omit(.x)),   # first non-NA per patient
      .names = "{.col}"
    ),
    .groups = "drop"
  )

nrow(clin_all)           # TMA-level rows
length(unique(clin_all$Patient))  # unique patients
nrow(clin_patient)  
...
# Harmonise names: make a TMA ID column in the mapping that matches master_features$tma_id
ss_patient_map2 <- ss_patient_map %>%
  rename(tma_id = SS, patient_id = Patient)

# Attach patient IDs to master_features using the TMA/SS mapping
master_features_with_patient <- master_features %>%
  left_join(ss_patient_map2, by = "tma_id")

master_patient <- master_features_with_patient %>%
  group_by(patient_id) %>%   # use patient_id here
  summarise(
    across(where(is.numeric), ~ mean(.x, na.rm = TRUE)),
    .groups = "drop"
  )

cat("Rows TMA level:", nrow(master_features_with_patient), "\n")
cat("Rows patient level:", nrow(master_patient), "\n")

###merging features and clin data
merged_df <- master_patient %>%
  left_join(clin_patient, by = "patient_id")


cat("Patients in master_patient:", nrow(master_patient), "\n")
cat("Patients in clin_patient:",  nrow(clin_patient),  "\n")
cat("Patients in merged_df:",     nrow(merged_df),     "\n")

# How many merged rows have no clinical data?
sum(is.na(merged_df$age))  # or another key clinical column




# 1. Convert yes/no event indicators to 0/1
merged_df <- merged_df %>%
  mutate(
    os_event  = ifelse(tolower(trimws(os_event))  == "yes", 1, 0),
    dss_event = ifelse(tolower(trimws(dss_event)) == "yes", 1, 0),
    dfs_event = ifelse(tolower(trimws(dfs_event)) == "yes", 1, 0)
  )

# 2. Ensure time and age columns are numeric
merged_df <- merged_df %>%
  mutate(
...
# 4. Quick checks
cat("os_event:\n");  print(table(merged_df_clean$os_event,  useNA = "always"))
...
cat("Patients before DFS filter:", nrow(merged_df), "\n")
cat("Patients after  DFS filter:", nrow(merged_df_dfs), "\n")
...
dfs_lasso <- run_lasso_cox(
  data          = merged_df_dfs,      # or merged_df_dfs if you filtered earlier
  time_col      = "dfs_months",
  event_col     = "dfs_event",
  endpoint_name = "DFS"
)
...
###redoing graph with shorter axis
library(survival)
library(survminer)
library(dplyr)

# 1. Reload merged_df with the prognosis groups
merged_df <- readRDS("merged_df_with_valley_groups.rds")

# 2. Recreate the KM dataset: only GOOD and POOR
km_df <- merged_df %>% 
  filter(prognosis_group %in% c("GOOD", "POOR"))

# 3. Recreate the Surv object and survfit model
surv_obj <- Surv(time = km_df$dss_months, event = km_df$dss_event)
km_fit   <- survfit(surv_obj ~ prognosis_group, data = km_df)

# 4. Plot again with a shorter x-axis (e.g. 0–60 months)
ggsurvplot(
  km_fit,
  data       = km_df,
  pval       = TRUE,
  ...
  title      = "KM: Natural Split by Prognostic Score",
  xlab       = "Time (months)",
  ylim       = c(0.75, 1.0)   # <---- shorter y-axis
)
